...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectNode(d, _that) {
        this.selectedNodeHtml = _that;
        this.$refs.detailPanel.showDetail(d, d3.select(_that).attr("r"));
      },
      ...
      changeNodeRadius(id, radius) {
        ...
        d3.select(this.selectedNodeHtml).attr("r", radius);
      },

      // TODO 重制后svg没有事件了 或许可以一开始生成rect 然后设置圆角来切换
      changeNodeShape(id, shape) {
        // circle2rect
        if (shape === 1) {
          let circle = document.getElementById("node" + id);
          let parent = circle.parentNode;
          let rect = document.createElement('rect');
          rect.setAttribute("id", "node" + id);
          rect.setAttribute("class", "node");
          rect.setAttribute("fill", circle.getAttribute("fill"));
          rect.setAttribute("x", circle.getAttribute("cx") * 1 - circle.getAttribute("r") * 1);
          rect.setAttribute("y", circle.getAttribute("cy") * 1 - circle.getAttribute("r") * 1);
          ...
          d3.select(".mainGraph").select("svg").html(d3.select(".mainGraph").select("svg").html());
        }
        // rect2circle
        else {
          let rect = document.getElementById("node" + id);
          let parent = rect.parentNode;
          let circle = document.createElement('circle');
          circle.setAttribute("id", "node" + id);
          circle.setAttribute("class", "node");
          circle.setAttribute("fill", rect.getAttribute("fill"));
          circle.setAttribute("cx", rect.getAttribute("x") * 1 + rect.getAttribute("height") / 2);
          circle.setAttribute("cy", rect.getAttribute("y") * 1 + rect.getAttribute("height") / 2);
          circle.setAttribute("r", rect.getAttribute("height") / 2);
