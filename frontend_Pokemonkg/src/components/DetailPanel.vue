...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeToRect(){
        if (d3.select("#node"+this.currentNodeId).attr("r")){
          this.$parent.changeNodeShape(this.currentNode.id,1);
          this.currentShapeIsRect = true;
        }
      },
      changeToCircle(){
        if (d3.select("#node"+this.currentNodeId).attr("width")){
          this.$parent.changeNodeShape(this.currentNode.id,0);
          this.currentShapeIsRect = false;
        }
      }
    }
  }
